...
if (empty($row->ean_info)) {
	...
	if ($resql && ($o = $db->fetch_object($resql))) {
		$db->query("UPDATE ".MAIN_DB_PREFIX."scan_capture SET ean_info = '".$db->escape($o->ean_info)."' WHERE rowid = ".((int) $row->rowid));
		print json_encode(array('ok' => true, 'cached' => true, 'shared' => true, 'info' => json_decode($o->ean_info)));
		exit;
	}
}
