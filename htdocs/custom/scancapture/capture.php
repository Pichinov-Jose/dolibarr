...
if ($resql) {
	while ($o = $db->fetch_object($resql)) {
		...
		if ($o->fk_product) {
			$lblcell = '<a href="'.DOL_URL_ROOT.'/product/card.php?id='.((int) $o->fk_product).'" target="_blank" class="classforajaxtooltip" data-params=\'{"id":'.((int) $o->fk_product).',"objecttype":"product"}\'><span class="fa fa-cube paddingright" style="color:#4a6785"></span>'.$lblcell.'</a>';
		}
...
jQuery(function() {
	...
	function submitRowParams(params, forced) {
		...
		jQuery.getJSON(base + 'saverow.php', params).fail(function() {
			...
			var lblcell = (r.fk_product ? '<a href="<?php print DOL_URL_ROOT; ?>/product/card.php?id=' + r.fk_product + '" target="_blank" class="classforajaxtooltip" data-params=\'{"id":' + r.fk_product + ',"objecttype":"product"}\'><span class="fa fa-cube paddingright" style="color:#4a6785"></span>' + (r.label || '') + '</a>' : (r.label || ''));
	...
	jQuery('#sc_cr_ok').on('click', function() {
		...
		jQuery.getJSON(base + ep, {rowid: crRow, label: jQuery('#sc_cr_label').val(), price: jQuery('#sc_cr_price').val(), buyprice: jQuery('#sc_cr_buyprice').val(), mpn: jQuery('#sc_cr_mpn').val(), parent_mode: jQuery('input[name=sc_cr_parent]:checked').val() || 'none', parent_label: jQuery('#sc_cr_parent_label').val(), specs_keep: specsKeep.join('||'), decl: dm, fk_supplier: jQuery('#sc_cr_supplier').val() || 0, token: token}).fail(function() {
			...
			if (r.fk_product) {
				tr.find('td').eq(3).html('<a href="<?php print DOL_URL_ROOT; ?>/product/card.php?id=' + r.fk_product + '" target="_blank" class="classforajaxtooltip" data-params=\'{"id":' + r.fk_product + ',"objecttype":"product"}\'><span class="fa fa-cube paddingright" style="color:#4a6785"></span>' + (r.label || '') + '</a>');
			} else {
