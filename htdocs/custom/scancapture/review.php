...
if ($action == 'masscreate') {
	...
	foreach ($ids as $id) {
		...
		$resql = $db->query("SELECT rowid, ean, qty, fk_inventory FROM ".MAIN_DB_PREFIX."scan_capture WHERE rowid = ".((int) $id)." AND status = 'unknown'");
...
$resql = $db->query("SELECT rowid, code_kezia, ean, qty, ean_info FROM ".MAIN_DB_PREFIX."scan_capture WHERE status = 'unknown' ORDER BY rowid DESC LIMIT 500");
$nb = 0;
if ($resql) {
	while ($o = $db->fetch_object($resql)) {
		...
		print '<td>'.dol_escape_htmltag(trim($brand.' '.$title)).($info === null ? '<span class="opacitymedium"> (pas encore interrogé)</span>' : ($title === '' ? '<span class="opacitymedium">aucun résultat</span>' : '')).'</td>';
