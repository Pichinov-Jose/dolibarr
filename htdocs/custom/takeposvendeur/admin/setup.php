...
if ($action == 'setvalue' && $user->admin) {
	...
	$ok = $ok && dolibarr_set_const($db, "TAKEPOSVENDEUR_COST_BASIS", GETPOST('TAKEPOSVENDEUR_COST_BASIS', 'aZ09'), 'chaine', 0, '', $conf->entity) >= 0;
...
$cb = getDolGlobalString('TAKEPOSVENDEUR_COST_BASIS', 'pmp');
...
$costarr = array('pmp' => $langs->trans("TpvCostPmp"), 'cost' => $langs->trans("TpvCostCostPrice"), 'supplier' => $langs->trans("TpvCostSupplier"));
print '<tr class="oddeven"><td>'.$langs->trans("TpvCostBasis").'<br><span class="opacitymedium small">'.$langs->trans("TpvCostBasisHelp").'</span></td>';
print '<td>'.$form->selectarray("TAKEPOSVENDEUR_COST_BASIS", $costarr, $cb).'</td></tr>';
